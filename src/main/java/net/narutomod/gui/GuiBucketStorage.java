...
public class GuiBucketStorage extends ElementsNarutomodMod.ModElement {
	public static int GUIID = 65;
	public static HashMap guistate = new HashMap();
	public GuiBucketStorage(ElementsNarutomodMod instance) {
		super(instance, 863);
	}
	...
	public static class GuiContainerMod extends Container implements Supplier<Map<Integer, Slot>> {
		...
		public void onContainerClosed(EntityPlayer playerIn) {
			...
			if (!bound && playerIn instanceof EntityPlayerMP) {
				...
				if (!playerIn.isEntityAlive() || playerIn instanceof EntityPlayerMP && playerMP.hasDisconnected()) {
					for (int j = 0; j < internal.getSlots(); ++j) {
						playerIn.entityDropItem(internal.extractItem(j, internal.getStackInSlot(j).getCount(), false), 0.0F);
					}
				} else {
					NonNullList<ItemStack> remainingItems = NonNullList.create();

					for (int i = 0; i < internal.getSlots(); ++i) {
						ItemStack remaining = internal.extractItem(i, internal.getStackInSlot(i).getCount(), false);
					...
					if (!remainingItems.isEmpty()) {
						for (ItemStack remaining : remainingItems) {
							playerIn.entityDropItem(remaining, 0.0F);
						}
					}
				}
			} else {
	...
	public static class GuiWindow extends GuiContainer {
		...
		private static final ResourceLocation texture = new ResourceLocation("narutomod:textures/bucket_storage.png");
		@Override
		...
		protected void drawGuiContainerForegroundLayer(int par1, int par2) {
			this.fontRenderer.drawString("Candy Bucket", 6, 11, -2195435);
		}
